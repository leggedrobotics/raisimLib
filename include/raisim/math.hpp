...
class Vec {
 public:
  ...
  typedef Eigen::Map<const Eigen::Matrix<double, n, 1> > ConstEigenVecn;
...
class Mat {
 public:
  ...
  typedef Eigen::Map<Eigen::Matrix<double, n, m> > EigenMat;
  typedef Eigen::Map<const Eigen::Matrix<double, n, m> > ConstEigenMat;
  ...
  const ConstEigenMat e() const{
    return ConstEigenMat(v);
  }
...
class SparseJacobian {
 public:
  ...
  void resize(size_t cols) {
    ...
    memset(v.ptr(), 0, cols*3 * sizeof(double));
  }
